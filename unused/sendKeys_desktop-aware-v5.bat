...
var psCommand =
    ...
    '$lookup=$t; ' +
    'if ($lookup.Length -ge 2 -and $lookup[0] -eq \'[\' -and $lookup[$lookup.Length-1] -eq \']\') { $lookup=$lookup.Substring(1,$lookup.Length-2) }; ' +
    '$h=Find-WindowHandle $lookup; ' +
    'if ($null -eq $h -or [int64]$h -eq 0) { exit 1 }; ' +
    '$targetDesktop=$h | Get-DesktopFromWindow; ' +
...
// Now that the target desktop is current, activate the original title.
